...
void PopulateItems(int boxId)
{
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW)
    {
        ...
        std::wstring witem(item, item + strlen(item));
        ...
        case WM_CREATE:
{
    ...
    // Buttons above ListView
    int btnX = treeWidth + 10;
    int btnY = 5; // just a little below top
    CreateWindow(L"BUTTON", L"Add Box", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON,
                 btnX, btnY, 100, buttonHeight, hwnd, (HMENU)101, GetModuleHandle(nullptr), nullptr);
    CreateWindow(L"BUTTON", L"Delete Box", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON,
                 btnX + 110, btnY, 100, buttonHeight, hwnd, (HMENU)102, GetModuleHandle(nullptr), nullptr);
    CreateWindow(L"BUTTON", L"Add Item", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON,
                 btnX + 220, btnY, 100, buttonHeight, hwnd, (HMENU)103, GetModuleHandle(nullptr), nullptr);
    CreateWindow(L"BUTTON", L"Delete Item", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON,
                 btnX + 330, btnY, 100, buttonHeight, hwnd, (HMENU)104, GetModuleHandle(nullptr), nullptr);

    // ListView starts below buttons
    int listY = btnY + buttonHeight + buttonSpacing;
    hList = CreateWindowEx(
        0, WC_LISTVIEW, L"",
        WS_CHILD | WS_VISIBLE | WS_BORDER | LVS_REPORT | LVS_SHOWSELALWAYS,
        treeWidth, listY, width - treeWidth, height - listY,
        hwnd, nullptr, nullptr, nullptr);
    ...
    col.cx = 300;
    col.pszText = (LPWSTR)L"Item";
    ListView_InsertColumn(hList, 0, &col);

    col.cx = 100;
    col.pszText = (LPWSTR)L"Quantity";
    ...
    break;
        ...
        case WM_COMMAND:
        {
            switch(LOWORD(wParam))
            {
                case 101: // Add Box
                {
                    wchar_t buf[256] = L"New Box";
                    addBox(db, std::string(buf, buf + wcslen(buf)));
                    PopulateBoxes();
                    break;
                }
                ...
                case 103: // Add Item
                {
                    ...
                    wchar_t buf[256] = L"New Item";
                    int itemId = addItem(db, std::string(buf, buf + wcslen(buf)));
                ...
                case 104: // Delete Item
                {
                    ...
                    std::string itemName(buf, buf + wcslen(buf));
        ...
        case WM_SIZE:
        {
            ...
            int btnY = 5;
            int spacing = 5;
            int listY = btnY + buttonHeight + spacing;

            MoveWindow(hTree, 0, 0, treeWidth, h, TRUE);
            MoveWindow(hList, treeWidth, listY, w - treeWidth, h - listY, TRUE);

            MoveWindow(GetDlgItem(hwnd, 101), treeWidth + 10, btnY, 100, buttonHeight, TRUE);
            MoveWindow(GetDlgItem(hwnd, 102), treeWidth + 120, btnY, 100, buttonHeight, TRUE);
            MoveWindow(GetDlgItem(hwnd, 103), treeWidth + 230, btnY, 100, buttonHeight, TRUE);
            MoveWindow(GetDlgItem(hwnd, 104), treeWidth + 340, btnY, 100, buttonHeight, TRUE);
